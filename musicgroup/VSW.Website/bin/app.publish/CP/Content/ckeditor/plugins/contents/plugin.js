...
function buildToc(element){
    ...
    element.setHtml('<p class="toc-title">Xem nhanh</p>');
    Container = new CKEDITOR.dom.element( 'ul' );
    ...
    for (var i = 0 ; i < length ; ++i) {
        ...
        if (diff > 0) {
            ...
            var ulNode = new CKEDITOR.dom.element( 'ul' );
        ...
        if (diff < 0) {
            while (0 !== diff++) {
                ...
                Container = (parent.getName() === 'ul' ? parent : Container);
            }
            parentLevel = newLevel;
        }
