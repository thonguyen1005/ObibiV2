...
<form id="vswForm" name="vswForm" method="post">
    ...
    <div class="page-content-wrapper">
        <h3 class="page-title">Đánh giá</h3>
                                ...
                                <table class="table table-striped table-hover table-bordered dataTable">
                                    <thead>
                                        <tr>
                                            ...
                                            <th class="sorting text-center w10p"><%= GetSortLink("Số sao", "Vote")%></th>
                                            <th class="sorting text-center desktop"><%= GetSortLink("Đối tượng", "ProductID")%></th>
                                            <th class="sorting text-center desktop"><%= GetSortLink("Nội dung", "Content")%></th>
                                            <th class="sorting text-center w15p desktop"><%= GetSortLink("Ngày đánh giá", "Created")%></th>
                                    ...
                                    <tbody>
                                        <%for (var i = 0; listItem != null && i < listItem.Count; i++)
                                            {
                                                int count = ModVoteService.Instance.CreateQuery().Where(o => o.ParentID == listItem[i].ID).Count().ToValue().ToInt();
                                        %>
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <input type="number" class="form-control" onchange="$('#cb<%=i %>').prop('checked', true);" name="Vote<%= listItem[i].ID %>" value="<%= listItem[i].Vote %>" />
                                            </td>
                                            <td class="text-center"><%= listItem[i].GetObject() %></td>
