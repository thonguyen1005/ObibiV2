...
{
    public class FormProductOldController : ModProductController
    {
        public override void ActionIndex(ModProductModel model)
        {
            ...
            if (model.Value > 0)
            {
                var listOld = ModProductOldService.Instance.GetAll(model.Value);
                if(listOld != null)
                {
                    list.ForEach(
                        x=>
                        {
                            x.Check = (listOld.Where(o => o.ProductOldID == x.ID).Count() > 0 ? true : false);
                        }
                    );
                }
            }
